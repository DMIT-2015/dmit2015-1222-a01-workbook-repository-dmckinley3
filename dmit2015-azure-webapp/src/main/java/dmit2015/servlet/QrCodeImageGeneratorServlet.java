...
public class QrCodeImageGeneratorServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (urlString == null || urlString.isBlank()) {
            response.getWriter().write("<strong>Missing url parameter</strong>");
        } else {
            String downloadImage = request.getParameter("download");
            if (downloadImage != null) {
                response.setHeader("Content-disposition", "attachment;filename=LinqQrCode");
            }
            try {
                ...
                e.printStackTrace();
                response.getWriter().write("Error generating QR Code image");
            }
